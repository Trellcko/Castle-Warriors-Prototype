...
namespace QFSW.QC
{
    public static class QuantumRegistry
    {
                ...
                if (_objectRegistry.ContainsKey(type))
                {
                    if (_objectRegistry[type].Contains(obj))
                        throw new ArgumentException(
                            $"Could not register object '{obj}' of type {type.GetDisplayName()} as it was already registered.");
        ...
        public static void DeregisterObject(Type type, object obj)
        {
            ...
            lock (_objectRegistry)
            {
                if (_objectRegistry.ContainsKey(type) && _objectRegistry[type].Contains(obj))
                    ...
                    throw new ArgumentException(
                        $"Could not deregister object '{obj}' of type {type.GetDisplayName()} as it was not found in the registry.");
            }
        }
        ...
        {
            foreach (var obj in GetRegistryContents(typeof(T))) yield return (T)obj;
        }
        ...
        public static IEnumerable<object> GetRegistryContents(Type type)
        {
            ...
            lock (_objectRegistry)
            {
                if (_objectRegistry.ContainsKey(type))
                {
                    var registry = _objectRegistry[type];
        ...
        public static void ClearRegistryContents(Type type)
        {
            ...
            lock (_objectRegistry)
            {
                if (_objectRegistry.ContainsKey(type)) _objectRegistry[type].Clear();
            }
        }
        ...
        {
            if (GetRegistrySize<T>() <= 0) return $"The registry '{typeof(T).GetDisplayName()}' is empty".Yield();
